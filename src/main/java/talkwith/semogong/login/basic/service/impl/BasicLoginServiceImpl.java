...
public class BasicLoginServiceImpl implements BasicLoginService {
    ...
    public ResponseResult validateLoginInfo(String email, String password) {
        ...
        if (findMember.isEmpty()) {
            return new ResponseResult(ResponseCode.NOT_GENERATED_CODE, "회원 정보 조회 샐패",
        ...
        if (!member.getPassword().equals(password)) {
            return new ResponseResult(ResponseCode.NOT_MATCHED_CODE, "패스워드 불일치",
                    Map.of("clientMsg","비밀번호가 일치하지않습니다."));
        }

        return new ResponseResult(ResponseCode.SUCCESS, "로그인 성공",
    ...
    public ResponseResult validateFindId(String name){
        ...
        if (findMember.isEmpty()){
            return new ResponseResult(ResponseCode.NOT_GENERATED_CODE, "존재하지 않는 계정",
                    Map.of("clientMsg","매칭되는 계정을 찾을 수 없습니다."));
        }

        Member member = findMember.get();

        return new ResponseResult(ResponseCode.SUCCESS, "일치하는 계정 발견",
    ...
    public ResponseResult validateFindPassword(String email){
        ...
        if (member.isEmpty()){
            return new ResponseResult(ResponseCode.NOT_GENERATED_CODE, "존재하지 않는 아이디",
        ...
        return new ResponseResult(ResponseCode.SUCCESS, "비밀번호 찾기 : 이메일 전송",
